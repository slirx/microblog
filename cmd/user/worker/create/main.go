...
func main() {
	...
	conf, err := NewConfig("USER_WORKER_CREATE_")
	if err != nil {
		zapLogger.Fatal(err)
	}
	...
	apmTracer.Service.Name = "user-worker-create"

	w := worker.NewWorker(
		"user/worker/create",
		rabbitmq.NewClient(conf.RabbitMQ, zapLogger, queue.JobUserCreate),
		zapLogger,
		apmTracer,
		user.NewCreateHandler(zapLogger, user.NewRepository(db), m),
		queue.JobUserCreate,
	)
	w.Run(ctx)
}
